...
| DELETE | `/api/other-items/:id` | 删除物品（级联删除相关记录） |
...
| DELETE | `/api/usage/logs/:id`  | 删除使用记录                  |
...
1. 在使用记录页面点击"📄 导出记录"按钮
2. 自动下载 CSV 文件（文件名：`使用记录_YYYY-MM-DD.csv`）
3. 可用 Excel 打开查看或编辑

**批量导入使用记录**

1. 准备 CSV 文件（可以用导出的文件作为模板）
2. CSV 格式：`姓名,物品类型,款式,尺寸,物品名称,数量,备注,时间`
3. 注意事项：
   - 物品类型填写：`衣服` 或 `其它`
   - 衣服类型：填写款式和尺寸，物品名称留空
   - 其它类型：填写物品名称，款式和尺寸留空
...
项目根目录提供了 `usage_template.csv` 模板文件供参考。
